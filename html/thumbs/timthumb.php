...
define ('CACHE_SIZE', 500);					// number of files to store before clearing cache
define ('CACHE_CLEAR', 300);					// maximum number of files to delete on each cache clear
...
$max_w =  (int) abs (get_request ('mw', 0));
$max_h =  (int) abs (get_request ('mh', 0));
$bc_max =  (int) abs (get_request ('s', 0));
...
if ($new_width == 0 && $new_height == 0  && $bc_max==0 &&$max_w==0 && $max_h==0) {
    $new_width = 100;
    $new_height = 100;
}
...
if (file_exists ($src)) {
	...
	if($bc_max!=0)
    {
		...
		{
			$new_width = $bc_max;
			$new_height = $bc_max/$imgratio;
		}
		else
		{
			$new_height = $bc_max;
			$new_width = $bc_max*$imgratio;
		}
    }
    elseif($max_w!=0 && $max_h!=0)
    {
    	...
    	$max_height=$max_h;
    	if($width<=$height) // VERTICAL O CUADRADA
    	{
